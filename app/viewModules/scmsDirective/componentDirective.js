...
export default (app, elem, attrs, scope) => {
    ...
    app.directive('componentViewDirective', [
      function() {
        return {
          template: html,
          restrict: 'EA',
            ...
            var parseData = function() {
              ...
              angular.forEach(item.scope, function(scope) {
                item.view +=
                  ...
                  scope.key +
                  '</span></td><td object-to-view-directive key-model="' +
                  scope.key +
                  '" data-scope-type="' +
                  scope.scopeType +
                  '" data-type="' +
                  ...
                  scope.key +
                  '" data-example-value="' +
                  (scope.exampleValue || scope.parentScopeValue || '') +
                  '" data-disabled="' +
                  (scope.isDisabled || '') +
                  '"></td></tr>';
              });
              ...
              $scope.$watch('item', function(newValue, oldValue) {
                if (newValue) {
                  ...
                  angular.forEach(newValue.scope, function(item) {
                    item.exampleValue =
                      item.exampleValue || item.parentScopeValue || '';
                    if (
                      item.exampleValue ||
                      item.hasOwnProperty('exampleValue')
                    ) {
                      ...
                      if (item.type === 'function' && item.parentScopeValue) {
                        $scope[item.key] = function() {
                          var value = item.exampleValue.replace(
                            /^\s*function\(\)\s*{/,
                            ''
                          );
